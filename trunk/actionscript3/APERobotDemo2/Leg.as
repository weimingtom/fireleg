...
package {
	...
	public class Leg extends org.cove.ape.JAGS.JAGSComposite {
		...
		//private var pa:RectangleParticle;
		//private var pb:RectangleParticle;
		//private var pc:RectangleParticle;
		...
		public function Leg (
			...
			//pa = new RectangleParticle(px + 31, py - 8, 2, 2, 0);
			//pb = new RectangleParticle(px + 25, py - 37, 2, 2, 0);
			//pc = new RectangleParticle(px + 60, py -15, 2, 2, 0);

			
			// bottom triangle particles -- pf is the foot
			pd = new CircleParticle(px + 72 * os, py + 12 * scale,  1);
			pe = new CircleParticle(px + 43 * os, py + 19 * scale,  4);
			pf = new CircleParticle(px + 54 * os, py + 61 * scale,  2);

			// trying to keep this thing on the ground!
			//pe.addForcePersistent(new VectorForce(false, 0, RobotDemo.footGravity));
		...
		// make at least one set collidable and it won't tump over as much
		public function makeLegCollidable():void {
			pa.collidable = true;
			pb.collidable = true;
			pc.collidable = true;
			pd.collidable = true;
		...
		public function get fix():AbstractParticle {
			return pa;
		}
		...
		public override function paint():void {
			...
			//sg.lineStyle(lineWeight, 0xFF0044, lineAlpha);
			//pa.paint();
